...
// =============================================
// SERVICE CATEGORY T&C — always from DB
// =============================================

/**
 * Detect which service categories are used in $items,
 * then load their T&C from site_content.
 * Returns array keyed by tnc_basic / tnc_sublimation / tnc_signage.
 */
function _loadCategoryTnc(array $items): array {
    $sublimationTypes = ['tshirt','t-shirt','polo-shirt','polo shirt','sport-jersey','sport jersey','short','long-sleeve','long sleeve','jacket','sublimation'];
    $signageTypes     = ['acrylic','stainless','panaflex','billboard','signage'];

    $needBasic = $needSub = $needSign = false;
    foreach ($items as $item) {
        ...
        if (preg_match('/sublimation|tshirt|t-shirt|polo|jersey|jacket|short|long.?sleeve/', $raw)) {
            $needSub = true;
        } elseif (preg_match('/signage|acrylic|stainless|panaflex|billboard/', $raw)) {
            $needSign = true;
        } else {
            $needBasic = true;
        }
    }
    if (!$needBasic && !$needSub && !$needSign) $needBasic = true;

    $keys = [];
    if ($needBasic) $keys[] = 'tnc_basic';
    if ($needSub)   $keys[] = 'tnc_sublimation';
    if ($needSign)  $keys[] = 'tnc_signage';

    $result = [];
    try {
        $db  = db();
        $in  = implode(',', array_fill(0, count($keys), '?'));
        $st  = $db->prepare("SELECT content_key, content_value FROM site_content WHERE content_key IN ($in)");
        $st->bind_param(str_repeat('s', count($keys)), ...$keys);
        $st->execute();
        foreach ($st->get_result()->fetch_all(MYSQLI_ASSOC) as $row) {
            $result[$row['content_key']] = trim($row['content_value']);
        }
    } catch (\Throwable $e) {}

    // Hardcoded fallbacks only if DB row is missing
    $fallbacks = [
    ...
    foreach ($keys as $k) {
        if (empty($result[$k])) $result[$k] = $fallbacks[$k];
    }

    // Preserve order: basic → sublimation → signage
    $ordered = [];
    foreach (['tnc_basic','tnc_sublimation','tnc_signage'] as $k) {
        if (isset($result[$k])) $ordered[$k] = $result[$k];
    }
    return $ordered;
}

/**
 * Render T&C map into HTML bullets, grouped by category label when multiple.
 */
function _renderTncHtml(array $tncMap): string {
    $labels = [
        'tnc_basic'       => 'Basic Services',
        'tnc_sublimation' => 'Sublimation Services',
        'tnc_signage'     => 'Signage Services',
    ];
    $multi = count($tncMap) > 1;
    $html  = '';
    foreach ($tncMap as $key => $text) {
        if (!trim($text)) continue;
        if ($multi) {
            $html .= "<p style='font-size:9.5px;font-weight:700;margin:8px 0 2px;text-transform:uppercase;letter-spacing:0.5px'>" . ($labels[$key] ?? '') . ":</p>";
        }
        foreach (explode("\n", $text) as $line) {
            $line = trim($line);
            if ($line === '') continue;
            $html .= "<p style='margin:0;padding-left:4px;font-size:9.5px;line-height:1.75;color:#333'>&#8226;" . htmlspecialchars($line) . "</p>";
        }
    }
    return $html;
}
...
function buildPDFHtml(array $q, array $items): string {
    ...
    if ($discAmt > 0) {
        $totalBlock =
            "<div style='text-align:right;padding:10px 8px 3px;font-size:11px;color:#333;border-top:1px solid #ddd'>Original Price = P" . number_format($subtotal, 2) . "</div>" .
            "<div style='text-align:right;padding:3px 8px 3px;font-size:11px;color:#c00'>Corporate Discount = P" . number_format($discAmt, 2) . "</div>" .
            "<div style='text-align:right;padding:6px 8px 6px;border-top:1px solid #ccc'><span style='font-size:18px;font-weight:900;letter-spacing:0.5px'>TOTAL PRICE P" . number_format($total, 2) . "</span></div>";
    } else {
        $totalBlock =
            "<div style='text-align:right;padding:14px 8px 6px'><span style='font-size:18px;font-weight:900;letter-spacing:0.5px'>TOTAL P" . number_format($total, 2) . "</span></div>";
    }

    // T&C: premium client uses their own; standard uses category-based from DB
    $customTnc = trim($q['terms_conditions'] ?? '');
    if ($customTnc) {
        $tncHtml = '';
        foreach (explode("\n", $customTnc) as $line) {
            $line = trim($line);
            if ($line === '') continue;
            $tncHtml .= "<p style='margin:0;padding-left:4px'>&#8226;" . htmlspecialchars($line) . "</p>";
        }
    } else {
        $tncMap  = _loadCategoryTnc($items);
        $tncHtml = _renderTncHtml($tncMap);
    }

    return _pdfTemplate($infoBlock, $dear, $rows, $totalBlock, $prepBy, $chkBy, $tncHtml);
}

function _pdfTemplate(string $infoBlock, string $dear, string $rows, string $totalBlock, string $prepBy, string $chkBy, string $tncHtml): string {
    $css = '
...
function buildRequestPDFHtml(array $req, array $items): string {
    ...
    // Load category T&C from DB based on selected services
    $tncMap  = _loadCategoryTnc($items);
    $tncHtml = _renderTncHtml($tncMap);
    $tncSection = $tncHtml ? "
      <div style='margin-top:20px;padding-top:12px;border-top:1px solid #ccc'>
        <p style='font-size:11px;font-weight:700;text-align:center;text-decoration:underline;text-transform:uppercase;margin-bottom:6px'>TERMS AND CONDITIONS</p>
        {$tncHtml}
      </div>" : '';
