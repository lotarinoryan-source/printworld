...
$sizeSlugs = ['tarpaulin', 'sintraboard'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
.form-group select:disabled,.form-group input:disabled{background:#f0f0f0;color:#999;cursor:not-allowed}
  ...
  <form method="POST" action="process_quotation.php" enctype="multipart/form-data" id="qt-form">
    ...
    <div class="qt-section">
      ...
      <div class="tab-pane" id="tab-signage">
        <div class="svc-grid">
          <?php foreach ($servicesByCategory['signage'] as $svc): ?>
          <div class="svc-card" onclick="selectService(this,'<?= htmlspecialchars($svc['slug']) ?>','<?= htmlspecialchars($svc['name']) ?>','signage')">
            <i class="fas <?= htmlspecialchars($svc['icon'] ?? 'fa-sign-hanging') ?>"></i><span><?= htmlspecialchars($svc['name']) ?></span>
          </div>
          <?php endforeach; ?>
        </div>
...
var SIGNAGE_OPTIONS = {
  'acrylic':   ['Flat Type','Build Up Type','Build Up Type with Cladding'],
  'stainless': ['Flat Type','Build Up Type','Build Up Type with Cladding'],
  'panaflex':  ['Single Face','Double Face','Single Frame','Double Face Frame','Special Design'],
  'billboard': ['Single Frame','Double Face Frame']
};
var ALWAYS_NONLIT = ['billboard'];
var NONLIT_OPTIONS = ['Single Frame','Double Face Frame'];
...
function renderBasic(div, slug, name) {
  ...
  if (unit) {
    sizeHtml =
      '<div class="form-row">' +
        '<div class="form-group" style="margin:0"><label>Width (' + unit + ') *</label><input type="number" id="cfg-w" min="0.1" step="0.1" placeholder="e.g. 4"></div>' +
        '<div class="form-group" style="margin:0"><label>Height (' + unit + ') *</label><input type="number" id="cfg-h" min="0.1" step="0.1" placeholder="e.g. 3"></div>' +
      '</div><br>';
  }
  var qtyHtml = !unit ?
    '<div class="form-row">' +
      '<div class="form-group" style="margin:0"><label>Quantity *</label><input type="number" id="cfg-qty" min="1" value="1"></div>' +
      '<div class="form-group" style="margin:0"><label>Notes (optional)</label><input type="text" id="cfg-notes" placeholder="e.g. red color, custom text"></div>' +
    '</div><br>' :
...
function addBasic(slug, name) {
  var unit = SIZE_BASIC[slug];
  var notes = document.getElementById('cfg-notes') ? document.getElementById('cfg-notes').value : '';
  var desc = name;
  var details = {};
  var qty = 1;
  if (unit) {
    ...
    desc += ' \u2014 ' + w + unit + ' \u00d7 ' + h + unit;
    details.width = w; details.height = h; details.unit = unit;
  } else {
...
function renderSublimation(div, slug, name) {
  div.innerHTML =
    ...
    '<div class="form-row">' +
      '<div class="form-group" style="margin:0"><label>Quantity *</label><input type="number" id="cfg-qty" min="1" value="1"></div>' +
      '<div class="form-group" style="margin:0"><label>Notes (optional)</label><input type="text" id="cfg-notes" placeholder="e.g. size, color preference"></div>' +
    '</div><br>' +
    designNote() +
...
function renderSignage(div, slug, name) {
  var opts = SIGNAGE_OPTIONS[slug] || ['Single Face','Double Face','Single Frame','Double Face Frame','Special Design'];
  var isBillboard = (slug === 'billboard');
  var alwaysNonLit = ALWAYS_NONLIT.indexOf(slug) !== -1;
  var optionsHtml = '';
  opts.forEach(function(o) { optionsHtml += '<option value="' + o + '">' + o + '</option>'; });
  var lightDisabled = alwaysNonLit ? ' disabled' : '';
  var lightNote = alwaysNonLit
    ? '<p class="lock-note">Billboard is always Non-lighted.</p>'
    : '<p class="lock-note" id="light-note" style="display:none">Single Frame / Double Face Frame is always Non-lighted.</p>';

  div.innerHTML =
    '<h4 style="margin:0 0 16px;font-size:.95rem">' + name + '</h4>' +
    (isBillboard ? '<div class="info-note" style="margin-bottom:16px"><i class="fas fa-info-circle"></i> Billboard only allows Single Frame or Double Face Frame, and is always Non-lighted.</div>' : '') +
    '<div class="form-row">' +
      '<div class="form-group" style="margin:0"><label>Signage Option *</label>' +
        '<select id="cfg-option" onchange="onSignageChange(\'' + slug + '\')">' + optionsHtml + '</select>' +
      '</div>' +
      '<div class="form-group" style="margin:0"><label>Light Option</label>' +
        '<select id="cfg-light"' + lightDisabled + '>' +
          '<option value="Lighted"' + (alwaysNonLit ? ' disabled' : '') + '>Lighted</option>' +
          '<option value="Non-lighted"' + (alwaysNonLit ? ' selected' : '') + '>Non-lighted</option>' +
        '</select>' +
        lightNote +
      '</div>' +
    '</div>' +
    ...
    '<button type="button" class="add-item-btn" onclick="addSignage(\'' + slug + '\',\'' + name + '\')"><i class="fas fa-plus"></i> Add to Request</button>';

  onSignageChange(slug);
}

function onSignageChange(slug) {
  ...
  if (ALWAYS_NONLIT.indexOf(slug) !== -1) return;
  var forceNonLit = NONLIT_OPTIONS.indexOf(optEl.value) !== -1;
  lightEl.disabled = forceNonLit;
  if (forceNonLit) lightEl.value = 'Non-lighted';
...
function addSignage(slug, name) {
  ...
  var h = parseFloat(document.getElementById('cfg-h').value) || 0;
  var notes = document.getElementById('cfg-notes').value;
  if (!opt) { alert('Please select a signage option.'); return; }
  if (!w || !h) { alert('Please enter width and height.'); return; }
  var desc = name + ' \u2014 ' + opt + ' \u2014 ' + light + ' \u2014 ' + w + 'ft \u00d7 ' + h + 'ft';
  if (notes) desc += ' \u2014 ' + notes;
...
function updateCart() {
  ...
  if (cart.length === 0) {
    ...
    cart.forEach(function(item, idx) {
      ...
      li.innerHTML =
        '<div style="flex:1">' +
          '<div class="cart-item-name">' + item.item_name +
            ' <span style="font-size:.75rem;color:#888;font-weight:400;text-transform:capitalize">(' + item.service_type + ')</span></div>' +
          '<div class="cart-item-desc">' + item.description + ' &mdash; Qty: ' + item.quantity + '</div>' +
        '</div>' +
